"""Warehouse Suggestion Agent — Central Europe (DACH region, France).

Competes with warehouse_agent_north and warehouse_agent_south via CNP bidding.
...
SYSTEM_PROMPT = """\
You are a Warehouse Suggestion Agent specializing in Central European logistics \
markets (Germany, France, DACH region). You receive warehouse inventory for your \
region, client requirements, and upstream context.
...
1. Match available warehouses against client requirements considering:
   ...
   - Special features (cold storage, hazmat, security, ESD flooring, clean rooms)
   - Central European logistics advantages (Autobahn network, EU trade hub \
     positioning, pharma/tech infrastructure)
...
def _get_region_filter() -> str:
    ...
    if _REGION_FILTER is None:
        _REGION_FILTER = os.getenv(
            "WAREHOUSE_REGION_FILTER",
            "Bavaria,Baden-Wurttemberg,Ile-de-France,Auvergne-Rhone-Alpes",
        )
...
async def run(rm, pipeline_run_id, to_task, triggered_by, message):
    ...
    try:
        ...
        sections.append(f"## Regional Warehouses (Central Europe)\n{rows_to_context(rows, 'warehouses')}")

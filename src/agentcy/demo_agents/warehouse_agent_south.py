"""Warehouse Suggestion Agent — Southern Europe (Italy, Mediterranean corridor).

Competes with warehouse_agent_north and warehouse_agent_central via CNP bidding.
...
SYSTEM_PROMPT = """\
You are a Warehouse Suggestion Agent specializing in Southern European logistics \
markets (Italy, Mediterranean corridor). You receive warehouse inventory for your \
...
1. Match available warehouses against client requirements considering:
   ...
   - Southern European logistics advantages (Mediterranean shipping routes, \
     Italian manufacturing corridors, food/fashion supply chains)
...
def _get_region_filter() -> str:
    ...
    if _REGION_FILTER is None:
        _REGION_FILTER = os.getenv("WAREHOUSE_REGION_FILTER", "Lombardy,Emilia-Romagna,Lazio")
...
async def run(rm, pipeline_run_id, to_task, triggered_by, message):
    ...
    try:
        ...
        sections.append(f"## Regional Warehouses (Southern Europe)\n{rows_to_context(rows, 'warehouses')}")

...
# ──────────────────────────────────────────────────────────────────────────────
# 2) derive host / port
#    support connection strings with query params like ?dns_srv=false
# ──────────────────────────────────────────────────────────────────────────────
CB_HOST_PORT="${CB_CONN_STR#couchbase://}"
CB_HOST_PORT="${CB_HOST_PORT%%\?*}"
CB_HOST="${CB_HOST_PORT%%:*}"
CB_PORT="${CB_HOST_PORT##*:}"
[[ -z "$CB_HOST" ]] && die "Unable to parse host from CB_CONN_STR=$CB_CONN_STR"
...
# ──────────────────────────────────────────────────────────────────────────────
if ! couchbase-cli server-list -c "$CLUSTER" -u "$CB_USER" -p "$CB_PASS" >/dev/null 2>&1; then
...
mkbucket "${EPHEMERAL_BUCKET:-$CB_BUCKET}" 128

# ──────────────────────────────────────────────────────────────────────────────
# 6) scope   (skip if _default or name starts with “_”)
# ──────────────────────────────────────────────────────────────────────────────
if [[ "$CB_SCOPE" != "_default" && "${CB_SCOPE:0:1}" != "_" ]]; then
  for B in "$CB_BUCKET" "${EPHEMERAL_BUCKET:-$CB_BUCKET}"; do
...
# ──────────────────────────────────────────────────────────────────────────────
#   accepted patterns (preferred):
#       CB_COLLECTIONS_<name>=foo
#       CB_COLLECTIONS__<name>=foo
#   and for ephemeral:
#       EPHEMERAL_COLLECTIONS_<name>=bar
#
#   FALLBACK (comma-separated):
#       CB_COLLECTIONS="agents,pipelines,..."
#       EPHEMERAL_COLLECTIONS="pipeline_runs_ephemeral,ephemeral_large_outputs"
#
...
# Fallback to comma-separated lists if none of the prefixed vars exist
if [[ ${#PERSISTENT_COLS[@]} -eq 0 && -n "${CB_COLLECTIONS:-}" ]]; then
  # Split on commas, trim whitespace, drop empties
  mapfile -t PERSISTENT_COLS < <(
    printf '%s' "$CB_COLLECTIONS" \
      | tr ',' '\n' \
...
if [[ ${#EPHEMERAL_COLS[@]} -eq 0 && -n "${EPHEMERAL_COLLECTIONS:-}" ]]; then
  mapfile -t EPHEMERAL_COLS < <(
    printf '%s' "$EPHEMERAL_COLLECTIONS" \
      | tr ',' '\n' \
      | sed -E 's/^[[:space:]]+|[[:space:]]+$//g' \
      | sed -n '/^.*[^[:space:]].*$/p'
  )
...
log "📑 creating *ephemeral* collections in ${EPHEMERAL_BUCKET:-$CB_BUCKET}.$CB_SCOPE …"
mkcols "${EPHEMERAL_BUCKET:-$CB_BUCKET}" "${EPHEMERAL_COLS[@]}"
